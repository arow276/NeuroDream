...
export default function NewDreamEntry({
  ...
  // Step 2: Emotions, symbols, themes
  ...
  const handleSave = () => {
    ...
    // Generate interpretation and wellness actions
      ...
      <div className="card p-6">
        ...
        {step === 2 && (
          <Step2_EmotionsAndSymbols
            ...
            setNewSymbolName={setNewSymbolName}
            newSymbolCategory={newSymbolCategory}
            setNewSymbolCategory={setNewSymbolCategory}
            addSymbol={addSymbol}
        ...
        {step === 4 && (
          <Step4_LifeContext
            ...
            setLifeContext={setLifeContext}
            stressorInput={stressorInput}
            setStressorInput={setStressorInput}
            currentStressors={currentStressors}
            addStressor={addStressor}
            removeStressor={(i) =>
              setCurrentStressors(currentStressors.filter((_, idx) => idx !== i))
            }
            ...
            addEvent={addEvent}
            removeEvent={(i) =>
              setRecentEvents(recentEvents.filter((_, idx) => idx !== i))
            }
          />
        )}
      </div>
    ...
    <div className="space-y-5">
      ...
      <div>
        ...
        <textarea
          ...
          placeholder="Describe your dream in as much detail as you can remember. Include settings, characters, actions, feelings, and any details that stood out..."
          rows={8}
...
function Step2_EmotionsAndSymbols({
  selectedEmotions,
  toggleEmotion,
  selectedThemes,
  toggleTheme,
  ...
  setNewSymbolName: (v: string) => void;
  newSymbolCategory: SymbolCategory;
  setNewSymbolCategory: (v: SymbolCategory) => void;
  addSymbol: () => void;
  removeSymbol: (i: number) => void;
}) {
  return (
            ...
            <button
              ...
              className={`tag cursor-pointer transition-all ${
                selectedEmotions.includes(emotion)
                  ? "!bg-dream-500/30 !text-dream-300 !border-dream-500/50"
        ...
        <div className="flex flex-wrap gap-2">
          {THEMES.map((theme) => (
            <button
              ...
              className={`tag tag-green cursor-pointer transition-all ${
                selectedThemes.includes(theme)
                  ? "!bg-neuro-500/30 !text-neuro-300 !border-neuro-500/50"
                  : "hover:border-neuro-400/40"
              }`}
            >
              {theme}
            </button>
      ...
      <div>
        <div className="flex items-center gap-2 mb-3">
          ...
          <span className="text-xs text-[var(--text-muted)]">
            (people, places, objects, animals that appeared)
          </span>
        </div>
          ...
          <div className="flex flex-wrap gap-2 mb-3">
            {symbols.map((sym, i) => (
              <span key={i} className="tag tag-amber flex items-center gap-1">
                {sym.name}
                <span className="text-[10px] opacity-60">({sym.category})</span>
                <button
        ...
        <div className="flex gap-2">
          ...
          <button onClick={addSymbol} className="btn btn-secondary shrink-0">
            <Plus size={14} />
          </button>
        </div>
      </div>
...
function Step3_SleepContext({
  ...
  sleepQuality: SleepQuality;
  setSleepQuality: (v: SleepQuality) => void;
  bedTime: string;
  setBedTime: (v: string) => void;
  wakeTime: string;
  setWakeTime: (v: string) => void;
  sleepStage: SleepStage;
  setSleepStage: (v: SleepStage) => void;
  lucidity: Lucidity;
  setLucidity: (v: Lucidity) => void;
  vividness: MoodRating;
  setVividness: (v: MoodRating) => void;
  emotionalIntensity: MoodRating;
  setEmotionalIntensity: (v: MoodRating) => void;
  valence: DreamValence;
  setValence: (v: DreamValence) => void;
  isRecurring: boolean;
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        <label className="block text-sm text-[var(--text-secondary)] mb-2">
          Estimated sleep stage (when did you think this dream occurred?)
        </label>
          ...
          {(["N1", "N2", "N3", "REM"] as SleepStage[]).map((stage) => (
            <button
              ...
              className={`p-3 rounded-lg border text-sm text-center transition-all ${
                sleepStage === stage
                  ? "border-dream-500 bg-dream-500/20 text-dream-300"
      ...
      <div>
        <label className="block text-sm text-[var(--text-secondary)] mb-2">
          ...
          Lucidity (were you aware you were dreaming?)
        </label>
        <div className="flex gap-2">
          {(["none", "partial", "full"] as Lucidity[]).map((luc) => (
            <button
              key={luc}
              onClick={() => setLucidity(luc)}
            ...
            >
              {luc === "none" ? "Not lucid" : luc === "partial" ? "Partially" : "Fully lucid"}
            </button>
          ))}
        </div>
      </div>

      {/* Vividness & emotional intensity */}
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div>
          ...
          <RatingSelector
            value={vividness}
            onChange={(v) => setVividness(v as MoodRating)}
            labels={["Faint", "Dim", "Clear", "Vivid", "Hyper-vivid"]}
          />
        </div>
        <div>
          <label className="block text-sm text-[var(--text-secondary)] mb-2">
          ...
          <RatingSelector
            ...
            onChange={(v) => setEmotionalIntensity(v as MoodRating)}
            labels={["Mild", "Low", "Moderate", "Intense", "Overwhelming"]}
          />
        </div>
      </div>
...
function Step4_LifeContext({
  ...
  lifeContext: string;
  setLifeContext: (v: string) => void;
  stressorInput: string;
  setStressorInput: (v: string) => void;
  currentStressors: string[];
  addStressor: () => void;
  removeStressor: (i: number) => void;
  eventInput: string;
  setEventInput: (v: string) => void;
  recentEvents: string[];
  addEvent: () => void;
  ...
  return (
    <div className="space-y-6">
      ...
      <p className="text-sm text-[var(--text-muted)] -mt-4">
        This information helps generate more personalized, context-aware
        interpretations. All fields are optional.
      </p>
      ...
      <div>
        ...
        <textarea
          ...
          placeholder="Briefly describe your current life situation — work, relationships, personal projects, transitions, challenges..."
          rows={3}
        />
      </div>
      ...
      <div>
        ...
        {currentStressors.length > 0 && (
          <div className="flex flex-wrap gap-2 mb-2">
            {currentStressors.map((s, i) => (
              <span key={i} className="tag tag-rose flex items-center gap-1">
                {s}
                <button onClick={() => removeStressor(i)} className="hover:text-white">
                  <X size={12} />
                </button>
              </span>
            ))}
        ...
        <div className="flex gap-2">
          ...
          />
          <button onClick={addStressor} className="btn btn-secondary shrink-0">
            <Plus size={14} />
          </button>
        </div>
      </div>
      ...
      <div>
        ...
        {recentEvents.length > 0 && (
          <div className="flex flex-wrap gap-2 mb-2">
            {recentEvents.map((e, i) => (
              <span key={i} className="tag tag-blue flex items-center gap-1">
                {e}
                <button onClick={() => removeEvent(i)} className="hover:text-white">
                  <X size={12} />
                </button>
              </span>
            ))}
        ...
        <div className="flex gap-2">
          <input
            ...
            placeholder="Add a recent event..."
            onKeyDown={(e) => e.key === "Enter" && addEvent()}
          />
          <button onClick={addEvent} className="btn btn-secondary shrink-0">
            <Plus size={14} />
          </button>
        </div>
      </div>
      ...
      {/* Context explanation */}
      <div className="p-4 rounded-lg bg-dream-500/10 border border-dream-500/20">
        <div className="flex items-start gap-3">
          <Brain size={18} className="text-dream-400 mt-0.5 shrink-0" />
          <div>
            <p className="text-sm text-dream-200 font-medium mb-1">
              How this helps
            </p>
            <p className="text-xs text-dream-300/70">
              Your life context allows NeuroDream to build context-aware prompts
              that connect your dream content to your waking experience. Research
              shows dream content closely mirrors waking concerns (the
              Continuity Hypothesis), so this context enables more meaningful
              interpretations and more relevant wellness suggestions.
            </p>
          </div>
        </div>
      </div>
